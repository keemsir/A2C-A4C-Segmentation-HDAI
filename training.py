...
def img_size(file_path: str = 'DB/train/A2C/'):
...
img_size('DB/train/A2C/')
img_size('DB/train/A4C/')
...
def png2nifti(training_folder: str = 'DB/train/A2C', test_folder: str = '/mnt/dataset/test_set',
              save_folder: str = 'DB_nifti/train/A2C'):
    ...
    t2_list = img_size('DB/train/A2C/') # training_folder
    ...
    for i in t2_list:
        ...
        t2_file_group.append(file_name)
    ...
    training_npys = subfiles('DB/train/A2C/', suffix='.npy', join=False) # training_folder


    for t in training_npys:

        tn = t[:-4]

        t_img = os.path.join('DB/train/A2C', tn + '.png')
        t_label = os.path.join('DB/train/A2C', tn + '.npy')
        pngTemp = plt.imread(t_img)
        npyTemp = np.load(t_label)
...
def json_mk(save_dir: str = 'DB_nifti/train/A2C'):
    # Path
